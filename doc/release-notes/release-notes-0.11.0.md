...
Moneta Core 0.11.0 is forked off the Bitcoin Core 0.9.3. The old moneta
...
with the  introduced changes to the Moneta Core 0.11.0.
...
- Defined BIP32 (HD) address versions to start with `drkp`/`drkv` (`DRKP`/`DRKV`
  for testnet)
- Adapted BIP44 coin type `5` for Moneta (0x80000005) as defined in SLIP-0044
