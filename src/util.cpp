...
void ParseParameters(int argc, const char* const argv[])
{
    ...
    BOOST_FOREACH(const PAIRTYPE(string,string)& entry, mapArgs)
    {
        ...
        if (name.find("--") == 0)
        {
            std::string singleMoneta(name.begin()+1, name.end());
            if (mapArgs.count(singleMoneta) == 0)
                mapArgs[singleMoneta] = entry.second;
            name = singleMoneta;
        }
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty moneta.conf if it does not excist
