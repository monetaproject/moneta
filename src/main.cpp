...
static const int64_t nTargetTimespan = 24 * 60 * 60; // Moneta: 1 day
static const int64_t nTargetSpacing = 2.5 * 60; // Moneta: 2.5 minutes
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, moneta - DarkGravity v3, written by Evan Duffield - evan@moneta.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
