...
const char* BITCOIN_REQUEST_MIMETYPE = "application/moneta-paymentrequest";
const char* BITCOIN_PAYMENTACK_MIMETYPE = "application/moneta-paymentack";
const char* BITCOIN_PAYMENTACK_CONTENTTYPE = "application/moneta-payment";
...
bool PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // moneta: URI
        {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // moneta: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
                ...
                emit message(tr("URI handling"),
                    tr("URI can not be parsed! This can be caused by an invalid Moneta address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
